...
public class LevelGeneratorEditor : UnityEditor.Editor
{
    ...
    private void GenerateMap()
    {
        for (var x = 0; x < generator.MapSize; x++)
        {
            ...
            {
                if (x == 0 || x == generator.MapSize -1 || y == 0 || y == generator.MapSize)
                {
                    map[x, y] = 1;
                }
                else
                {
                    var filled = Random.Range(0, 1f) < generator.FillPercentage ? 1 : 0;
                    map[x, y] = filled;
                }
            }
        }
    ...
    private void CombineLightProbes()
    {
        ...
        foreach (var lightProbeGroup in lightProbeGroups)
        {
            ...
            {
                var adjustProbePosition = groupRotation * probePosition + groupPosition;
                probePositions.Add(adjustProbePosition);
            }
